window.__ModuleLoader__.load({ id: 'ming-life', factory: (require) => {
  ...
  const WORKBENCH_ID = 'wb-dataelement-dsh-ming-life'
  // Must equal package.json and workbench.json; tests/test-original-adapter.mjs checks it.
  const WORKBENCH_VERSION = '0.1.1'
  ...
  function activeSession(sessionId, project = openState.project) {
    const state = bridge.service.getSnapshot().state
    return state.active === WORKBENCH_ID && state.added.includes(WORKBENCH_ID) &&
      bridge.sessions.list.getSnapshot().current === sessionId &&
      state.sessionBindings[sessionId] === WORKBENCH_ID && openState.project === project
  }
  ...
  function sendNow(sessionId, prompt, project = openState.project) {
    if (bridge.service.getSnapshot().state.sessionBindings[sessionId] !== WORKBENCH_ID) return Promise.reject(new Error('会话不属于当前工作台'))
  ...
  async function ensureSession(slug, result) {
    ...
    const task = (async () => {
      ...
      suppressFollow++
      let sessionId
      try { sessionId = await bridge.service.ensureSession({ workbenchId: WORKBENCH_ID, folder: result.folder, sessionId: saved }) }
  ...
  function retryCurrent() {
    ...
    if (!slug || bridge.service.getSnapshot().state.active !== WORKBENCH_ID) return
  ...
  async function openProject(slug) {
    ...
    if (openState.project === slug && bridge.service.getSnapshot().state.active === WORKBENCH_ID)
  ...
  function Panel() {
    ...
    React.useEffect(() => {
      const onMessage = async event => {
        if (event.origin !== window.location.origin || event.source !== iframeRef.current?.contentWindow || event.data?.project !== openState.project || bridge.service.getSnapshot().state.active !== WORKBENCH_ID) return
  ...
  function apply(ctx) {
    ...
    ctx.effect(() => {
      ...
      let last = read()
      const check = () => { if (suppressFollow || bridge.service.getSnapshot().state.active !== WORKBENCH_ID) return; const id = read(); if (id === last) return; last = id; const slug = projectForSession(id); if (slug && slug !== openState.project) setOpen({ project: slug }) }
    ...
    ctx.effect(() => ctx.desktopWorkbenches.register({
      id: WORKBENCH_ID, title: '玄学人生工作台', icon: '☯', panelTitle: '人生档案',
      description: '本地排盘、人生档案与原版自动解读流程。',
      version: WORKBENCH_VERSION, author: 'dataelement / liqingb0220-stack', embedded: true,
  ...
  module.exports = { inject, apply, ensureSession, openProject, fillDraft, sendNow, flushPending, activeSession }
