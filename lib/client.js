window.__ModuleLoader__.load({ id: 'ming-life', factory: (require) => {
  ...
  function sendNow(sessionId, prompt, project = openState.project) {
    if (!activeSession(sessionId, project)) return Promise.reject(new Error('工作台或会话已切换'))
      ...
      const onMessage = async event => {
        ...
        if (data?.type === 'dsh-ming-life:ask') {
          try {
            ...
            if (!activeSession(sessionId, data.project)) throw new Error('工作台、档案或会话已切换')
  ...
  function apply(ctx) {
    ...
    ctx.effect(() => ctx.desktopWorkbenches.register({
      ...
      version: '1.2.0', author: 'dataelement / liqingb0220-stack', embedded: true,
